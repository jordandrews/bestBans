...
class BanCalculatorService {
            ...
            champList.each { String champName ->
                ...
                if(!champ) {    //if not Add our new champData to the database
                    champ = new ChampData(tier: tier,
                            ...
                            pickrate: pickrate,
                            createDate: today,
                            patchNumber: "5.22", //TODO: patch number here patchNumber: #
                            region: region
                    )
                }
    ...
    def assignRanks(ServerRegions region) {
        ...
        RankTiers.each { RankTiers tier ->
            ...
            def i = 1
            ChampData.findAllByTierAndRegionAndPatchNumberAndCreateDate(tier, region, '5.22',  today).sort{-it.influence}.each{ ChampData champ ->
                champ.rank = i
